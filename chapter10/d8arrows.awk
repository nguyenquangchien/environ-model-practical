...
#
# Usage: gawk -f d8arrows.awk -v size=value input [> output]
#
# Variables:
# ----------
...
END{
   # Number of arrow used in 3D visualization
   arrow_number=1;  
   ...
   for(this_E=min_E+size;this_E<max_E;this_E+=size){
      ...
      for(this_N=min_N+size;this_N<max_N;this_N+=size){
         ...
         printf("set arrow %i from %6i,%6i,%6.1f to \
            %6i,%6i,%6.1f", arrow_number, \
            this_E, this_N, dem[this_E,this_N], \
            this_E+steep_E, this_N+steep_N, \
            dem[this_E+steep_E,this_N+steep_N]);
         printf(" nohead front lt 1 lw 2.0\n");
         ++arrow_number;   # Increment arrow number		 
